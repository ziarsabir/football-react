  ...
  return (
     <section id="news" className="container mx-auto px-4 py-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-2 mb-4">
        ...
        {/* PL-only toggle */}
        <label className="flex items-center gap-2 sm:ml-2 bg-slate-100">
          <input
            type="checkbox"
            checked={plOnly}
        ...
        <div className="sm:ml-2 w-full sm:w-auto">
          <select
            ...
            className="w-full sm:w-64 rounded-xl border border-slate-300 bg-white/90 px-3 py-2 text-sm shadow-sm outline-none focus:border-slate-500 focus:ring-2 focus:ring-blue-200 disabled:opacity-50"
          >
            {PL_CLUBS.map((name) => (
        ...
        <div className="sm:ml-auto w-full sm:w-auto">
          <input
            ...
            className="w-full sm:w-72 rounded-xl border border-slate-300 bg-white/90 px-3 py-2 text-sm shadow-sm outline-none focus:border-slate-500 focus:ring-2 focus:ring-blue-200"
          />
        </div>
      </div>
            ...
            return (
              ...
              >
                ...
                {img ? (
                  ...
                  <div className="mb-3 h-2 w-10 rounded bg-gradient-to-r from-slate-500 to-slate-600" />
                )}

                {/* Accent bar */}
                <div className="h-1 w-12 rounded bg-gradient-to-r from-slate-200 to-slate-300 mb-2 opacity-80 group-hover:opacity-100" />

                {/* Title */}
                <h3 className="text-lg md:text-xl font-semibold leading-snug tracking-tight text-slate-900 group-hover:text-slate-700 line-clamp-2">
                  {title}
                </h3>
                ...
                <div className="mt-2 flex items-center gap-2 text-xs text-slate-500">
                  {showSource && (
                    <span className="px-2 py-0.5 rounded-full bg-slate-200 border border-slate-300 font-medium">
                      {it.source}
                    </span>
                  )}
                  {showDate && (
